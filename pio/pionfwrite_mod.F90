...
#   include <pnetcdf.inc>   /* _EXTERNAL */
    ...
    real(r4) , intent(in), pointer     :: IOBUF(:)
    ...
    if(file%iosystem%ioproc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          call pio_buffer_add_data (File, IOBUF)
#else
	  call pio_buffer_check(File,size(iobuf)*sizeof(iobuf(1)))
          ierr=nfmpi_bput_vara_real ( File%fh,varDesc%varid,start, &
               count, IOBUF , request )
          call pio_buffer_add_request(File,request)
...
#   include <pnetcdf.inc>   /* _EXTERNAL */
    ...
    type (File_desc_t), intent(inout) :: File
    integer(i4) , intent(in), pointer     :: IOBUF(:)
    type (var_desc_t), intent(in)     :: varDesc
    type (IO_desc_t), intent(in)      :: IODesc
...
#else
	  call pio_buffer_check(File,size(iobuf)*sizeof(iobuf(1)))
          ierr=nfmpi_bput_vara_int ( File%fh,varDesc%varid,start, &
               count, IOBUF , request )
          call pio_buffer_add_request(File,request)
...
#   include <pnetcdf.inc>   /* _EXTERNAL */
    ...
    type (File_desc_t), intent(inout) :: File
    real(r8) , intent(in), pointer     :: IOBUF(:)
    type (var_desc_t), intent(in)     :: varDesc
    type (IO_desc_t), intent(in)      :: IODesc
...
#else
	  call pio_buffer_check(File,size(iobuf)*sizeof(iobuf(1)))
          ierr=nfmpi_bput_vara_double ( File%fh,varDesc%varid,start, &
               count, IOBUF , request )
          call pio_buffer_add_request(File,request)
