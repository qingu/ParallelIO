...
!<  
# 71 "piodarray.F90.in"
  subroutine write_darray_1d_vdc (File,varDesc,ioDesc, array, iostat, fillval)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       call mpi_bcast({MPITYPE}, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
...
!! @param iostat : The status returned from this routine (see \ref PIO_seterrorhandling for details)
!<
# 142 "piodarray.F90.in"
  subroutine read_darray_1d_vdc (File,varDesc, ioDesc, array, iostat)
...
# 1205 "piodarray.F90.in"
 subroutine write_vdc2_real(File,varDesc,ioDesc,array, iostat)
    ...
    if (IOproc) then
	call write_vdc2_var(iobuf, start, count,  File%iosystem%IO_comm, varDesc%ts, varDesc%lod, varDesc%reflevel, TRIM(varDesc%name), File%iosystem%num_iotasks)
...
subroutine read_vdc2_real(File,varDesc,ioDesc,array, iostat)
    ...
    if(UseRearranger) then 
       if (IOproc) then 
          ...
          call alloc_check(IOBUF,len,' TYPE :IOBUF')
          IOBUF= -1.0_r8
  	  call read_vdc2_var(iobuf, start, count,  File%iosystem%IO_comm, varDesc%ts, varDesc%lod, varDesc%reflevel, TRIM(varDesc%name), File%iosystem%num_iotasks)
       else
          call alloc_check(IOBUF,0)
          IOBUF= -1.0_r8
       endif
