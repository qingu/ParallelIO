...
# 404 "piodarray.F90.in"
  subroutine write_darray_nf_real (HFile,varDesc,ioDesc,array, iostat, fillval)
    ...
    type (File_desc_t), intent(inout), target  :: &
         HFile                   ! file information

    type (var_desc_t), intent(inout), target :: &
         varDesc                      ! variable descriptor
    type (io_desc_t), intent(inout) :: &
...
!<
# 404 "piodarray.F90.in"
  subroutine write_darray_nf_int (HFile,varDesc,ioDesc,array, iostat, fillval)
    use pio_timeseries, only : check_if_timeseries_file
    ! !DESCRIPTION:
    !  Writes a 2-d slab of TYPE to a netcdf file.
    !
...
!<
# 404 "piodarray.F90.in"
  subroutine write_darray_nf_double (HFile,varDesc,ioDesc,array, iostat, fillval)
    use pio_timeseries, only : check_if_timeseries_file
    ! !DESCRIPTION:
    !  Writes a 2-d slab of TYPE to a netcdf file.
    !
...
#   include <pnetcdf.inc>   /* _EXTERNAL */    
    ...
    if(file%iotype /= PIO_IOTYPE_PNETCDF) return

    if(associated(file%data_list_top) .and. file%request_cnt>0) then
#ifdef MEMCHK	
