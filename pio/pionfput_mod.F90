...
module pionfput_mod
  ...
  use pio_buffer, only: pio_buffer_add_request
#ifdef _PNETCDF
...
#elif defined( USE_BLOCKING_WRITE )
             ierr = nfmpi_put_vara_all (File%fh, varid, int(index,kind=PIO_OFFSET), int(count,kind=PIO_OFFSET), &
                  ival, int(count,kind=PIO_OFFSET), MPI_CHARACTER)
#else
             ierr = nfmpi_bput_vara_text (File%fh, varid, int(index,kind=PIO_OFFSET), int(count,kind=PIO_OFFSET), &
                  ival, request)
             call pio_buffer_add_request(file, request)
#endif
...
#else
          if(Ios%io_rank==0) then
             ierr = nfmpi_bput_var1_int (File%fh, varid, int(index,kind=PIO_OFFSET), ival, request)
             call pio_buffer_add_request(file, request)
          endif
#endif
#endif
...
#else
          if(Ios%io_rank==0) then
             ierr = nfmpi_bput_var1_real (File%fh, varid, int(index,kind=PIO_OFFSET), ival, request)
             call pio_buffer_add_request(file, request)
          endif
#endif
#endif
...
#else
          if(Ios%io_rank==0) then
             ierr = nfmpi_bput_var1_double (File%fh, varid, int(index,kind=PIO_OFFSET), ival, request)
             call pio_buffer_add_request(file, request)
          endif
#endif
#endif
    ...
    if(Ios%IOProc) then
	  ...
	  is=1
       ...
       case(pio_iotype_pnetcdf)
          ...
          if(Ios%io_rank==0 ) then
             ierr = nfmpi_bput_var_text (File%fh, varid, ival, request)
          end if
          call pio_buffer_add_request(File, request)
#endif
#endif
...
# 437 "pionfput_mod.F90.in"
  ...
  use mpi   ! _EXTERNAL
    ...
    type (File_desc_t), intent(inout) :: File
    integer, intent(in) :: varid
    integer(i4), intent(in), target :: ival
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          request = MPI_REQUEST_NULL
!          if(Ios%io_rank==0 ) then
!             ierr = nfmpi_bput_var_int (File%fh, varid, ival, request)
!          end if
#if(0==0)       
          if(Ios%io_rank==0 ) then
             ierr = nfmpi_bput_var1_int (File%fh, varid, int(start,kind=PIO_OFFSET), &
                  ival, request)
          end if
#else
          ierr = nfmpi_bput_vara_int (File%fh, varid, int(start,kind=PIO_OFFSET), &
               int(count,kind=PIO_OFFSET), ival, request)
#endif
          call pio_buffer_add_request(File, request)
  ...
  use mpi   ! _EXTERNAL
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          request = MPI_REQUEST_NULL
!          if(Ios%io_rank==0 ) then
!             ierr = nfmpi_bput_var_real (File%fh, varid, ival, request)
!          end if
#if(0==0)       
          if(Ios%io_rank==0 ) then
             ierr = nfmpi_bput_var1_real (File%fh, varid, int(start,kind=PIO_OFFSET), &
                  ival, request)
          end if
#else
          ierr = nfmpi_bput_vara_real (File%fh, varid, int(start,kind=PIO_OFFSET), &
               int(count,kind=PIO_OFFSET), ival, request)
#endif
          call pio_buffer_add_request(File, request)
  ...
  use mpi   ! _EXTERNAL
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          request = MPI_REQUEST_NULL
!          if(Ios%io_rank==0 ) then
!             ierr = nfmpi_bput_var_double (File%fh, varid, ival, request)
!          end if
#if(0==0)       
          if(Ios%io_rank==0 ) then
             ierr = nfmpi_bput_var1_double (File%fh, varid, int(start,kind=PIO_OFFSET), &
                  ival, request)
          end if
#else
          ierr = nfmpi_bput_vara_double (File%fh, varid, int(start,kind=PIO_OFFSET), &
               int(count,kind=PIO_OFFSET), ival, request)
#endif
          call pio_buffer_add_request(File, request)
...
#else
             request=MPI_REQUEST_NULL
             ierr = nfmpi_bput_vara_text (File%fh, varid, pstart, &
                  pcount, ival, request)
             call pio_buffer_add_request(File,request)
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          ierr = nfmpi_put_vara_int_all (File%fh, varid, pstart, &
               pcount, ival, clen, MPI_INTEGER)
#else
          ierr = nfmpi_bput_vara_int (File%fh, varid, pstart, &
               pcount, ival, request)
          call pio_buffer_add_request(File, request)
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          ierr = nfmpi_put_vara_real_all (File%fh, varid, pstart, &
               pcount, ival, clen, MPI_REAL4)
#else
          ierr = nfmpi_bput_vara_real (File%fh, varid, pstart, &
               pcount, ival, request)
          call pio_buffer_add_request(File, request)
    ...
    if(Ios%IOProc) then
       ...
       case(pio_iotype_pnetcdf)
          ...
          ierr = nfmpi_put_vara_double_all (File%fh, varid, pstart, &
               pcount, ival, clen, MPI_REAL8)
#else
          ierr = nfmpi_bput_vara_double (File%fh, varid, pstart, &
               pcount, ival, request)
          call pio_buffer_add_request(File, request)
