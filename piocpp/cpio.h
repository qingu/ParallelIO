...
void pio_cpp_init_intracom( int comp_rank, int comp_comm, int num_tasks, int num_aggregator,
...
void pio_cpp_init_intercom( int component_count, int peer_comm, int* comp_comms, int io_comm, void* iosystem);

// subroutine pio_cpp_finalize( iosystem, ierr) bind( c)

void pio_cpp_finalize( void* iosystem, int* ierror);
...
void pio_cpp_initdecomp_dof_i8( void* iosystem, int basepiotype, int* dims, int ndims, int* compdof, int ncompdof,
...
int pio_cpp_openfile( void* iosystem, void* file, int iotype, char* fname, int mode);

// subroutine pio_cpp_syncfile( file) bind( c)

void pio_cpp_syncfile( void* file);

// function pio_cpp_createfile( iosystem, file, iotype, fname, amode_in) result( ierr) bind( c)

int pio_cpp_createfile( void* iosystem, void* file, int iotype, char* fname, int amode_in);

// subroutine pio_cpp_closefile( file) bind( c)

void pio_cpp_closefile( void* file);

// subroutine pio_cpp_setiotype( file, iotype, rearr) bind( c)

void pio_cpp_setiotype( void* file, int iotype, int rearr);

// function pio_cpp_numtoread( iodesc) result( num) bind( c)

int pio_cpp_numtoread( void* iodesc);

// function pio_cpp_numtowrite( iodesc) result( num) bind( c)

int pio_cpp_numtowrite( void* iodesc);

// subroutine pio_cpp_setframe( vardesc, frame) bind( c)

void pio_cpp_setframe( void* vardesc, int frame);

// subroutine pio_cpp_advanceframe( vardesc) bind( c)

void pio_cpp_advanceframe( void* vardesc);
